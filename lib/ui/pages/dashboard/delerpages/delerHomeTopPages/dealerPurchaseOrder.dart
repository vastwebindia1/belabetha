...
class DealerPurchaseOrder extends StatefulWidget {
  const DealerPurchaseOrder({Key key}) : super(key: key);

  @override
  _DealerPurchaseOrderState createState() => _DealerPurchaseOrderState();
}

class _DealerPurchaseOrderState extends State<DealerPurchaseOrder> {
  var balance = "";
  ...
  Future<void> dealeruserdetails() async {
    ...
    if (response.statusCode == 200) {
      ...
      final decrypted3 =
      encrypter.decrypt(encrypt.Encrypted.fromBase64(photoo), iv: iv);

      final decrypted4 =
      encrypter.decrypt(encrypt.Encrypted.fromBase64(adminfirmnam), iv: iv);
    ...
    if (response.statusCode == 200) {
     ...
     /* if(upislabstatus1 == "Y"){
      ...
      Navigator.push(
          ...
          builder: (_) => DealerPaymentOption(amount:amountController.text,debitcharge2: debitcharge1.toString(),debitnet2: debitnet1.toString(),
