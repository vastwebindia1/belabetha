...
class debitgetway extends StatefulWidget {
  final String amount,
      debitamnt,
      debitnet,
      mercentid,
  ...
  const debitgetway(
      ...
      this.merchkey,
      this.succurl,
      this.failurl, this.uniqid})
      : super(key: key);

  @override
  _debitgetwayState createState() => _debitgetwayState();
}

class _debitgetwayState extends State<debitgetway> {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
        ...
        home: UserInputPage(
          ...
          debitamnt: widget.debitamnt,
          debitnet: widget.debitnet,
          succurl: widget.succurl,
...
class UserInputPage extends StatefulWidget {
  final String amount, debitamnt, debitnet, succurl, failurl,uniqid;
                        ...
                        child: Column(
                          children: [
                            Container(
                              ...
                              child: Image.asset('assets/pngImages/Debitcard.png'),
                            ),
                            SizedBox(height: 5,),
                            Text(getTranslated(context, 'Debit Card01'),style: TextStyle(color: TextColor,fontSize: 18,),)
                          ],
                        ),
                      ),
                    ],
...
/*  @override
  void onInitiatePaymentFailure(
      ...
      String udf4 ,
      String udf5 ) {
    print('$orderID -- $transactionID -- $paymentID -- $paymentStatus -- $date -- $time -- $paymode-- $amount -- $udf1 -- $udf2 -- $udf3 -- $udf4 -- $udf5');
    Utility.showSnackBarMessage(
        state: _scaffoldKey.currentState,
        message: '$orderID -- $transactionID -- $paymentID -- $paymentStatus -- $date -- $time -- $paymode-- $amount -- $udf1 -- $udf2 -- $udf3 -- $udf4 -- $udf5');
  }
