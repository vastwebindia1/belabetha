...
class Upimoneytransfer extends StatefulWidget {
  final String accnum, name,uniqidd;
  const Upimoneytransfer({Key key, this.accnum, this.name, this.uniqidd}) : super(key: key);

  @override
  _UpimoneytransferState createState() => _UpimoneytransferState();
}

class _UpimoneytransferState extends State<Upimoneytransfer> {
    ...
    try{
      ...
      if (response.statusCode == 200) {
        ...
        Navigator.pushReplacement(
          context,
          PageRouteBuilder(
            pageBuilder: (context, animation1, animation2) => Upiresponse(bankname: bnknamee,accnum:acccno ,bankrrn: bnkrrid,status: statuss,ifsc: "UPI",date: dateortime,mode: "UPI ID",amount: amountt,sservicefee: servicefee.text,name: widget.name,),
                      ...
                      onPressed: btnclick == false ? null : ()async{
                        ...
                        if(adhrrr == "true"  && aadharnaun < 12){
                          ...
                          if(enteramnt != reenteramntt){
                            ...
                            sendmoneydetails(sendnum, "null", "123456", amount.text, transpin.text, widget.accnum, "UPI", androidid, "null", ipadd, lat, long, modelname, address, city, postcode, netname);
