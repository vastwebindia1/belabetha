...
import 'Addnewwalletid.dart';
import 'WalletSender.dart';
import 'Walletsendmoney.dart';

class walletsenderlist extends StatefulWidget {
  const walletsenderlist({Key key}) : super(key: key);

  @override
  _walletsenderlistState createState() => _walletsenderlistState();
}

class _walletsenderlistState extends State<walletsenderlist> {
  bool editText = false;
  bool loader = false;
  ...
  Future<void> remeterdetails(String number) async {
    ...
    var url1 = new Uri.http(
        "api.vastwebindia.com", "/WALLET/api/WALLET/GetBeneficiaryList", {
    ...
    if (response.statusCode == 200) {
      ...
      if (status == "0") {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => NumberRegister()),
        );
      }else{
    ...
    if (response.statusCode == 200) {
      ...
      if (status == "TXN") {
        ...
        Navigator.pushReplacement(
          context,
          PageRouteBuilder(
            pageBuilder: (context, animation1, animation2) => walletsenderlist(),
  ...
  Future<void> deletebeneficiary(String idno) async {
    ...
    var url1 = new Uri.http("api.vastwebindia.com", "/WALLET/api/WALLET/DeleteBeneficiary", {
      "idno": idno,
    });
                                      ...
                                      child: TextButton(
                                        onPressed:(){
                                          Navigator.pushReplacement(
                                            context,
                                            PageRouteBuilder(
                                              pageBuilder: (context, animation1, animation2) => walletsenderpage(),
                                              transitionDuration: Duration(seconds: 0),
                                            ),);
                                        },
                                        child: Column(
                                            ...
                                            Expanded(
                                              child:  TextButton(
                                                ...
                                                onPressed: () {
                                                  ...
                                                  String accnum = benificiarylist[index]["walletid"];


                                                  genuniqeid(name,accnum);
