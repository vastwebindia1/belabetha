...
class CashatposPage extends StatefulWidget {
  const CashatposPage({Key key}) : super(key: key);

  @override
  _CashatposPageState createState() => _CashatposPageState();
}

class _CashatposPageState extends State<CashatposPage> {

  bool _validate = false;
  bool _isloading = false;
  var trnsidd,loginid,password;
  bool isnewlog;
  bool ismicroatm = false;
  ...
  Future<void> Apitransitions(String trnsid, String amnt, String ipadd, String type, String lat,String long, String model, String city, String pin,String intype,String address,var key, var iv) async {
    ...
    try{
      ...
      var uri =
      new Uri.http("api.vastwebindia.com", "/MICROATM/api/data/Apitransitions", {
        "Transtionid": trnsid,
        "Amount": amnt,
        "IPaddressss": ipadd,
        "Type": type,
...
*/
  ...
  Widget build(BuildContext context) {
    ...
    final snackBar = SnackBar(
      content: Text('Amount Should be between \u{20B9} 200 To \u{20B9} 2000',style: TextStyle(color: Colors.yellowAccent),),
    );
    return Material(
      child: SafeArea(
        child: Scaffold(
            ...
            child: Column(
              children: [
                                        ...
                                        child: Column(
                                          children: [
                                            Container(
                                              ...
                                              child: Image.asset('assets/pngImages/cashPos.png'),
                                            ),
                                            SizedBox(height: 5,),
                                            Text(getTranslated(context, 'cashposh'),style: TextStyle(color: TextColor,fontSize: 18,),)
                                          ],
                                        ),
                                      ),
                                    ],
                ...
                Container(
                  ...
                  child: Text(getTranslated(context, '1 CASH'),textAlign: TextAlign.justify,),
                ),
                SizedBox(
                  height: 20,
                ),
                Container(
                  margin: EdgeInsets.only(left: 10,right: 10),
                  child: Text(getTranslated(context, '2 CASH'),textAlign: TextAlign.justify,),
                ),
                SizedBox(
                  height: 20,
                ),
                      ...
                      Expanded(
                        child: OutlinedButton(
                          ...
                          onPressed:btnclick == false ? null :()async{
                            ...
                            if(amount.text == "" || amnt < 200 || amnt >2001){
                              ...
                              Apitransitionsencrypt(trnsidd, amount.text, ipadd, "cash", lat, long, modelname, city, postcode, netname, address);
                    ...
                    GestureDetector(
                      onTap: (){
                        Navigator.pushReplacement(
                            context,
                            PageRouteBuilder(
                              pageBuilder: (context, animation1, animation2) => MatmMainPage(),
                              transitionDuration: Duration(seconds: 0),
                            ),);
                      },
                      child: Container(
                        width:100 ,
                          ...
                          child: Text(getTranslated(context, 'Withdraw'),style: TextStyle(color: PrimaryColor),overflow: TextOverflow.ellipsis,)
                      ),
                    )
                  ],
                )
